...
/*   Updated: 2024/06/12 02:02:32 by bfleitas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

double scale(double input, double old_max, double new_min, double new_max)
{
...
void    put_in_buffer(int x, int y, t_fractal *fractal)
{
    ...
    z.imaginary = 0;
    i = 0;
    c.real = scale(x, WIDTH, fractal->min.real, fractal->max.real);
    c.imaginary = scale(y, HEIGHT, fractal->min.imaginary, fractal->max.imaginary);
